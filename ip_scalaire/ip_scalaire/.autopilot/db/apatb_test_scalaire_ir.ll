...
entry:
  ...
  %res_copy = alloca [9 x float], align 512
  %0 = bitcast float* %A to [256 x float]*
  %1 = bitcast float* %B to [256 x float]*
  %2 = bitcast float* %res to [9 x float]*
  call fastcc void @copy_in([256 x float]* %0, [256 x float]* nonnull align 512 %A_copy, [256 x float]* %1, [256 x float]* nonnull align 512 %B_copy, [9 x float]* %2, [9 x float]* nonnull align 512 %res_copy)
  ...
  %5 = getelementptr inbounds [9 x float], [9 x float]* %res_copy, i32 0, i32 0
  call void @apatb_test_scalaire_hw(float* %3, float* %4, float* %5)
  call fastcc void @copy_out([256 x float]* %0, [256 x float]* nonnull align 512 %A_copy, [256 x float]* %1, [256 x float]* nonnull align 512 %B_copy, [9 x float]* %2, [9 x float]* nonnull align 512 %res_copy)
  ret void
}

; Function Attrs: argmemonly noinline
define internal fastcc void @copy_in([256 x float]* readonly, [256 x float]* noalias align 512, [256 x float]* readonly, [256 x float]* noalias align 512, [9 x float]* readonly, [9 x float]* noalias align 512) unnamed_addr #1 {
entry:
...
for.loop:                                         ; preds = %for.loop, %copy
  %for.loop.idx3 = phi i64 [ 0, %copy ], [ %for.loop.idx.next, %for.loop ]
  %dst.addr.gep1 = getelementptr [9 x float], [9 x float]* %0, i64 0, i64 %for.loop.idx3
  %5 = bitcast float* %dst.addr.gep1 to i8*
  %src.addr.gep2 = getelementptr [9 x float], [9 x float]* %1, i64 0, i64 %for.loop.idx3
  %6 = bitcast float* %src.addr.gep2 to i8*
  call void @llvm.memcpy.p0i8.p0i8.i64(i8* align 1 %5, i8* align 1 %6, i64 4, i1 false)
  %for.loop.idx.next = add nuw nsw i64 %for.loop.idx3, 1
...
; Function Attrs: argmemonly noinline
define internal fastcc void @copy_out([256 x float]*, [256 x float]* noalias readonly align 512, [256 x float]*, [256 x float]* noalias readonly align 512, [9 x float]*, [9 x float]* noalias readonly align 512) unnamed_addr #4 {
entry:
...
entry:
  ...
  %5 = bitcast float* %2 to [9 x float]*
  call void @copy_out([256 x float]* null, [256 x float]* %3, [256 x float]* null, [256 x float]* %4, [9 x float]* null, [9 x float]* %5)
  %6 = bitcast [256 x float]* %3 to float*
  %7 = bitcast [256 x float]* %4 to float*
  %8 = bitcast [9 x float]* %5 to float*
  call void @test_scalaire_hw_stub(float* %6, float* %7, float* %8)
  call void @copy_in([256 x float]* null, [256 x float]* %3, [256 x float]* null, [256 x float]* %4, [9 x float]* null, [9 x float]* %5)
  ret void
}
