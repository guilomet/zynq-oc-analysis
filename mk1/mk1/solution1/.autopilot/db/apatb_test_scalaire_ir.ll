; ModuleID = 'F:/projet_m2/mk1/mk1/solution1/.autopilot/db/a.g.ld.5.gdce.bc'
...
entry:
  ...
  %2 = bitcast float* %res to [1 x float]*
  call fastcc void @copy_in([256 x float]* %0, [256 x float]* nonnull align 512 %A_copy, [256 x float]* %1, [256 x float]* nonnull align 512 %B_copy, [1 x float]* %2, [1 x float]* nonnull align 512 %res_copy)
  ...
  %5 = getelementptr inbounds [1 x float], [1 x float]* %res_copy, i32 0, i32 0
  call void @apatb_test_scalaire_hw(float* %3, float* %4, float* %5)
  call fastcc void @copy_out([256 x float]* %0, [256 x float]* nonnull align 512 %A_copy, [256 x float]* %1, [256 x float]* nonnull align 512 %B_copy, [1 x float]* %2, [1 x float]* nonnull align 512 %res_copy)
  ret void
}

; Function Attrs: argmemonly noinline
define internal fastcc void @copy_in([256 x float]* readonly, [256 x float]* noalias align 512, [256 x float]* readonly, [256 x float]* noalias align 512, [1 x float]* readonly, [1 x float]* noalias align 512) unnamed_addr #1 {
entry:
...
; Function Attrs: argmemonly noinline
define internal fastcc void @copy_out([256 x float]*, [256 x float]* noalias readonly align 512, [256 x float]*, [256 x float]* noalias readonly align 512, [1 x float]*, [1 x float]* noalias readonly align 512) unnamed_addr #4 {
entry:
...
entry:
  ...
  call void @copy_out([256 x float]* null, [256 x float]* %3, [256 x float]* null, [256 x float]* %4, [1 x float]* null, [1 x float]* %5)
  ...
  call void @copy_in([256 x float]* null, [256 x float]* %3, [256 x float]* null, [256 x float]* %4, [1 x float]* null, [1 x float]* %5)
  ret void
}
